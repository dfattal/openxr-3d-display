...
cam3d_quat_rotate(struct xrt_quat q, struct xrt_vec3 v)
{
...
static void
cam3d_mat4_from_quat(float *m, struct xrt_quat q)
{
...
static void
cam3d_build_view_matrix(float *out, struct xrt_quat orientation, struct xrt_vec3 eye_world)
{
	float rot[16];
                       ...
                       Camera3DView *out_views)
{
	...
	for (uint32_t i = 0; i < count; i++) {
		...
		struct xrt_vec3 eye_world = cam3d_quat_rotate(cam_ori, eye_local);
