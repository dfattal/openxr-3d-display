...
#include "m_display3d_view.h" // reuse Display3DScreen, display3d_apply_eye_factors
...
typedef struct Camera3DStereoView {
	...
	struct xrt_fov fov;          //!< Asymmetric FOV angles in radians (per-eye)
	struct xrt_vec3 eye_world;   //!< Eye position in world space
	struct xrt_quat orientation; //!< Camera orientation (same for both eyes)
} Camera3DStereoView;
