                 ...
                 const struct vk_device_features *optional_device_features)
{
	...
	if (!u_extension_list_contains(device_ext_list, "VK_KHR_portability_subset")) {
		VkExtensionProperties *props = NULL;
		uint32_t prop_count = 0;
		VkResult pret = vk_enumerate_physical_device_extension_properties(
		    vk, vk->physical_device, NULL, &prop_count, &props);
		if (pret == VK_SUCCESS) {
			...
			if (needs_portability) {
				const char *const *old_data = u_extension_list_get_data(device_ext_list);
				uint32_t old_size = u_extension_list_get_size(device_ext_list);
				struct u_extension_list_builder *builder =
				    u_extension_list_builder_create_with_capacity(old_size + 1);
				u_extension_list_builder_append_array(builder, old_data, old_size);
				u_extension_list_builder_append_unique(builder, "VK_KHR_portability_subset");
				struct u_extension_list *new_list = u_extension_list_builder_build(&builder);
				u_extension_list_destroy(&device_ext_list);
				device_ext_list = new_list;
				VK_DEBUG(vk, "Auto-enabling required device ext VK_KHR_portability_subset");
			}
		}
	}
