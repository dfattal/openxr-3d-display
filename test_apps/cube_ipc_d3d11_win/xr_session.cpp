...
/*!
 * @file
 * @brief  OpenXR session management (IPC mode, with XR_EXT_display_info)
 *
 * IPC variant: app code is identical to hosted — IPC is runtime-transparent.
...
bool InitializeOpenXR(XrSessionManager& xr) {
    ...
    strcpy_s(createInfo.applicationInfo.applicationName, "SRCubeOpenXR_IPC");
    ...
    LOG_INFO("OpenXR initialization complete (IPC mode)");
    return true;
}

bool CreateSession(XrSessionManager& xr, ID3D11Device* d3d11Device) {
    LOG_INFO("Creating OpenXR session (IPC, DisplayXR creates window)...");
