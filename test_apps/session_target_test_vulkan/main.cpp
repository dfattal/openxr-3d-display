...
/*!
 * @file
 * @brief  Main entry point for XR_EXT_session_target test application (Vulkan)
 *
 ...
 * using Vulkan as the graphics API.
 */
...
// Window class name
static const wchar_t* WINDOW_CLASS = L"SessionTargetTestVulkanClass";
static const wchar_t* WINDOW_TITLE = L"XR_EXT_session_target Test (Vulkan)";
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow) {
    ...
    LOG_INFO("=== XR_EXT_session_target Test Application (Vulkan) ===");
    ...
    // Create OpenXR session with Vulkan device and window handle (using XR_EXT_session_target)
    ...
    if (!CreateSession(xr, renderer.instance, renderer.physicalDevice, renderer.device,
        renderer.graphicsQueueFamily, 0, hwnd)) {
