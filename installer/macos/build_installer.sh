...
if [ -f "$ARTIFACT_DIR/bin/cube_vk_macos" ]; then
    "$SCRIPT_DIR/create_app_bundle.sh" "$ARTIFACT_DIR" "$WORK_DIR/SimCubeOpenXR.app"
    ...
    echo "Warning: cube_vk_macos not found, skipping test app component"
    HAS_TESTAPP=false
fi
